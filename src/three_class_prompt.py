"""Step 6: three-class sentiment prompt (POSITIVE / NEUTRAL / NEGATIVE).

Same design discipline as src/sentiment_prompt.py (Step 1): title/text delimited as
untrusted data, prompt-injection resistance, no rating in the payload. Kept as a separate
file so Step 1's original binary prompt stays intact as its own artifact; this is the
prompt "carried through" per PLAN.md Section 11's redefinition to three classes.
"""
import os
...
Classify the overall sentiment as exactly one of: POSITIVE, NEUTRAL, or NEGATIVE.
NEUTRAL means the review is genuinely mixed, lukewarm, or indifferent, neither clearly
positive nor clearly negative, not simply "not extreme."
...
Respond with exactly one JSON object on a single line, with exactly one key "label",
whose value is exactly "POSITIVE", "NEUTRAL", or "NEGATIVE" (uppercase). Output nothing
else: no explanation, no markdown, no code fences, no text before or after the JSON
object.
